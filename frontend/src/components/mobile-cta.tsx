  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 border-t bg-white/95 p-3 shadow-soft backdrop-blur md:hidden">
      <div className="grid grid-cols-3 gap-2">
        ...
        <a className="flex items-center justify-center gap-2 rounded-full bg-sky-500 px-3 py-3 text-sm font-black text-white" href={contacts.telegram.href}><Send className="h-4 w-4" /> Telegram</a>
        <a className="flex items-center justify-center gap-2 rounded-full bg-copper-500 px-3 py-3 text-sm font-black text-white" href={contacts.max.href}><MessageCircle className="h-4 w-4" /> MAX</a>
      </div>
    </div>
  );
}
