    ...
    <header className="sticky top-0 z-50 border-b border-border/70 bg-card/85 backdrop-blur-xl">
      <div className="container flex h-20 min-w-0 items-center justify-between gap-3 py-3 xl:gap-4">
        ...
        <nav className="hidden shrink-0 items-center gap-1 xl:flex" aria-label="Основная навигация">
          {nav.map((item) => (
            <Link key={item.href} href={item.href} className="rounded-full px-3 py-2 text-sm font-bold text-muted-foreground transition hover:bg-muted hover:text-foreground 2xl:px-4">
              {item.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="hidden min-w-0 shrink-0 items-center justify-end gap-2 xl:flex 2xl:gap-3">
          {primaryPhone ? (
            <a href={primaryPhone.href} className="hidden max-w-40 truncate rounded-full px-3 py-2 text-sm font-black text-foreground transition hover:bg-muted hover:text-copper-500 2xl:inline-block" aria-label={`Позвонить: ${primaryPhone.label}`}>
              {primaryPhone.label}
            </a>
          ) : null}
          ...
          <details className="group relative hidden 2xl:block">
            ...
            <div className="absolute right-0 top-full mt-2 w-56 rounded-2xl border border-border bg-card p-2 text-card-foreground shadow-card">
              {phones.map((phone) => (
                <a key={phone.href} href={phone.href} className="block truncate rounded-xl px-3 py-2 text-sm font-black transition hover:bg-muted hover:text-copper-500">
                  {phone.label}
                </a>
              ))}
              <a href={telegramHref} className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-black transition hover:bg-muted hover:text-copper-500" aria-label="Написать в Telegram">
                <Send className="h-4 w-4 shrink-0" />
                <span className="truncate">Telegram</span>
              </a>
              <a href={maxHref} className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-black transition hover:bg-muted hover:text-copper-500" aria-label="Написать в MAX">
          ...
          <a href={telegramHref} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-foreground transition hover:bg-muted hover:text-copper-500 2xl:hidden" aria-label="Написать в Telegram">
          ...
          <Button asChild variant="copper" className="shrink-0"><a href="#contacts"><Phone className="h-4 w-4" />Заказать замер</a></Button>
        </div>

        <div className="flex shrink-0 items-center gap-2 xl:hidden">
          <ViewModeSwitcher className="max-w-[calc(100vw-8rem)] overflow-x-auto" />
      ...
      <div className={cn("container grid overflow-hidden transition-all xl:hidden", open ? "max-h-[38rem] pb-4" : "max-h-0")}>
        <div className="rounded-[1.5rem] border border-border bg-card p-3 text-card-foreground shadow-card">
          {nav.map((item) => (
            <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className="block truncate rounded-2xl px-4 py-3 font-bold text-foreground hover:bg-muted">
              {item.label}
            </Link>
          ))}
          ...
          <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-4">
            ...
            <a href={telegramHref} onClick={() => setOpen(false)} className="flex min-w-0 items-center justify-center gap-2 rounded-2xl bg-sky-500 px-3 py-3 text-sm font-black text-steel-900">
              <Send className="h-4 w-4 shrink-0" />
              <span className="truncate">Telegram</span>
            </a>
