      ...
      <div className="container flex h-20 items-center justify-between gap-4 py-3">
        <Link href="/" className="flex items-center gap-3" aria-label={settings.company_name}>
          ...
          <span className="leading-tight">
            <span className="block text-base font-black uppercase tracking-wide text-foreground">{settings.company_name}</span>
            <span className="text-xs font-semibold text-muted-foreground">навесы под ключ в Крыму</span>
          </span>
        </Link>
        <nav className="hidden items-center gap-1 lg:flex">
          {nav.map((item) => (
            <Link key={item.href} href={item.href} className="rounded-full px-4 py-2 text-sm font-bold text-muted-foreground transition hover:bg-muted hover:text-foreground">
              {item.label}
            </Link>
          ))}
        </nav>
