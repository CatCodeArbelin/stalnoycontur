...
export function QuickContact({ settings = fallbackSettings }: { settings?: PublicSettings }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 hidden flex-col gap-3 md:flex">
      ...
      <a className="grid h-14 w-14 place-items-center rounded-full bg-sky-500 text-steel-900 shadow-soft" href={telegramHref} aria-label="Написать в Telegram"><Send className="h-5 w-5" /></a>
      <a className="grid h-14 w-14 place-items-center rounded-full bg-copper-500 text-steel-900 shadow-soft" href={maxHref} aria-label="Написать в MAX"><MessageCircle className="h-5 w-5" /></a>
      <a className="grid h-14 w-14 place-items-center rounded-full bg-lime-400 text-steel-900 shadow-soft" href={avitoHref} aria-label="Открыть профиль на Avito"><ShoppingBag className="h-5 w-5" /></a>
    </div>
  );
}
