...
const siteSettingsFields: SiteSettingsField[] = [
  ...
  { key: "phones", description: "Список телефонов для шапки, футера и быстрых контактов" },
  { key: "telegram", description: "Ссылка на Telegram" },
  { key: "max", description: "Ссылка на MAX" },
  { key: "cities", description: "Города обслуживания" },
  { key: "personal_data_consent_text", description: "Текст согласия на обработку персональных данных" },
];
...
function normalizePhones(value: unknown): PhoneRow[] {
  ...
  return value
    ...
    .map((item) => ({ label: normalizeString(item.label), href: normalizeString(item.href) }))
...
function formValueToSettingValue(key: SiteSettingsKey, form: SiteSettingsFormState) {
  if (key === "phones") {
    return form.phones
...
export function SiteSettingsForm() {
  ...
  function applySettings(items: AdminSetting[]) {
    ...
    for (const field of siteSettingsFields) {
      ...
      if (setting) nextForm[field.key] = settingValueToFormValue(field.key, setting.value) as never;
    }
    setSettings(items);
    setForm(nextForm);
  }
  ...
  async function saveSettings(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      for (const field of siteSettingsFields) {
        ...
        await request(existing ? `/admin/settings/${existing.id}` : "/admin/settings", { method: existing ? "PATCH" : "POST", body });
      }
  ...
  function updatePhone(index: number, key: keyof PhoneRow, value: string) {
    setForm((current) => ({
      ...current,
      phones: current.phones.map((phone, phoneIndex) => (phoneIndex === index ? { ...phone, [key]: value } : phone)),
    }));
  }

  function addPhone() {
    setForm((current) => ({ ...current, phones: [...current.phones, { label: "", href: "" }] }));
  }

  function removePhone(index: number) {
    setForm((current) => ({ ...current, phones: current.phones.filter((_, phoneIndex) => phoneIndex !== index) }));
  }
      ...
      <div className="container grid gap-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-3xl font-black text-steel-950 md:text-4xl">Основные настройки сайта</h1>
            <p className="mt-2 max-w-3xl text-steel-600">Управление публичными контактами, городами и текстом согласия без ручного ввода системных ключей.</p>
          </div>
          ...
          <Card>
            ...
            <CardContent>
              <form className="grid gap-4 md:grid-cols-3" onSubmit={login}>
                <input className="rounded-2xl border p-3" value={username} onChange={(event) => setUsername(event.target.value)} placeholder="Логин" />
                <input className="rounded-2xl border p-3" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Пароль" type="password" />
        ...
        {isAuthenticated ? (
          <Card>
            <CardHeader className="flex-row items-center justify-between gap-4">
              ...
              <Button variant="outline" size="sm" onClick={loadSettings}>Обновить</Button>
            </CardHeader>
            <CardContent>
              <form className="grid gap-5" onSubmit={saveSettings}>
                <label className="grid gap-1 text-sm font-semibold text-steel-700">
                  Название компании
                  <input className="rounded-2xl border p-3 font-normal" value={form.company_name} onChange={(event) => setForm((current) => ({ ...current, company_name: event.target.value }))} placeholder="Стальной Контур" />
                </label>

                <label className="grid gap-1 text-sm font-semibold text-steel-700">
                  Основной телефон
                  <input className="rounded-2xl border p-3 font-normal" value={form.phone} onChange={(event) => setForm((current) => ({ ...current, phone: event.target.value }))} placeholder="+7 (978) 000-00-00" />
                </label>

                <div className="grid gap-3 rounded-2xl border p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold text-steel-700">Список телефонов</p>
                      <p className="text-xs text-steel-500">Label отображается на сайте, href используется в ссылке tel:.</p>
                    </div>
                    <Button type="button" variant="outline" size="sm" onClick={addPhone}>Добавить телефон</Button>
                  </div>
                  <div className="grid gap-3">
                    {form.phones.map((phone, index) => (
                      <div className="grid gap-2 rounded-2xl bg-slate-100 p-3 md:grid-cols-[1fr_1fr_auto]" key={index}>
                        <input className="rounded-2xl border p-3" value={phone.label} onChange={(event) => updatePhone(index, "label", event.target.value)} placeholder="+7 (978) 000-00-00" aria-label="Телефон" />
                        <input className="rounded-2xl border p-3" value={phone.href} onChange={(event) => updatePhone(index, "href", event.target.value)} placeholder="tel:+79780000000" aria-label="Ссылка телефона" />
                        <Button type="button" variant="outline" size="sm" onClick={() => removePhone(index)}>Удалить</Button>
                      </div>
                    ))}
                    {!form.phones.length ? <p className="rounded-2xl bg-slate-100 p-4 text-sm text-steel-500">Телефоны пока не добавлены.</p> : null}
                  </div>
                </div>

                <label className="grid gap-1 text-sm font-semibold text-steel-700">
                  Telegram-ссылка
                  <input className="rounded-2xl border p-3 font-normal" value={form.telegram} onChange={(event) => setForm((current) => ({ ...current, telegram: event.target.value }))} placeholder="https://t.me/..." />
                </label>

                <label className="grid gap-1 text-sm font-semibold text-steel-700">
                  MAX-ссылка
                  <input className="rounded-2xl border p-3 font-normal" value={form.max} onChange={(event) => setForm((current) => ({ ...current, max: event.target.value }))} placeholder="https://max.ru/..." />
                </label>

                <label className="grid gap-1 text-sm font-semibold text-steel-700">
                  Города
                  <textarea className="min-h-32 rounded-2xl border p-3 font-normal" value={form.cities} onChange={(event) => setForm((current) => ({ ...current, cities: event.target.value }))} placeholder="Симферополь\nСевастополь\nЯлта" />
                  <span className="text-xs font-normal text-steel-500">Один город на строку.</span>
                </label>

                <label className="grid gap-1 text-sm font-semibold text-steel-700">
                  Текст согласия на обработку персональных данных
                  <textarea className="min-h-32 rounded-2xl border p-3 font-normal" value={form.personal_data_consent_text} onChange={(event) => setForm((current) => ({ ...current, personal_data_consent_text: event.target.value }))} />
                </label>

                <div className="flex flex-wrap gap-2">
                  <Button type="submit" disabled={isSaving}>{isSaving ? "Сохранение..." : "Сохранить настройки"}</Button>
                  <Button type="button" variant="outline" onClick={loadSettings}>Отменить изменения</Button>
                </div>
              </form>
            </CardContent>
          </Card>
