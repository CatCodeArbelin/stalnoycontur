...
type AdminResourceProps = AdminResourceConfig;
...
function toPayload(fields: Field[], form: Record<string, unknown>) {
  return Object.fromEntries(
    fields.map((field) => {
      ...
      if (field.type === "json")
        return [field.key, parseJsonField(field, value)];
...
type AdminCrudPanelProps = AdminResourceConfig & {
...
export function AdminCrudPanel({
  ...
  async function saveItem(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setForm(
        Object.fromEntries(
          fields.map((field) => [field.key, emptyValue(field)]),
        ),
      );
  ...
  function uploadCategory(fieldKey: string) {
    ...
    if (field?.type === "image" || field?.type === "image-list")
      return field.uploadCategory ?? "uploads";
    return "uploads";
  }
  ...
  async function uploadImage(fieldKey: string, file: File | null) {
    ...
    try {
      ...
      const data = await request("/admin/upload", {
        method: "POST",
        body: payload,
      });
      const url =
  ...
  async function uploadImageList(fieldKey: string, files: FileList | null) {
    ...
    try {
      ...
      setForm((current) => {
        ...
        return {
          ...current,
          [fieldKey]: JSON.stringify([...currentUrls, ...urls], null, 2),
        };
      });
    } catch (error) {
      showMessage(await formatApiError(error), "error");
  ...
  return (
    <div className="grid gap-6">
      <div>
        <h2 className="text-2xl font-black text-steel-950">{title}</h2>
        ...
        <Card className="overflow-hidden">
          ...
          <CardContent className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              <thead className="text-xs uppercase text-steel-500">
                <tr>
                  {columns.map((column) => (
            ...
            <p
              className={cn(
                "py-6 text-center text-steel-500",
                items.length && "hidden",
              )}
            >
              Записей пока нет.
