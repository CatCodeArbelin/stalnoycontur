...
from app.schemas.landing_page import PublicLandingPage
from app.services.content import (
    DEFAULT_CASES,
    DEFAULT_FAQ,
    DEFAULT_GALLERY_ITEMS,
    assemble_public_settings,
)
...
@router.get("/content/landing-pages/{slug}", response_model=PublicLandingPage)
@router.get("/landing-pages/{slug}", response_model=PublicLandingPage)
def get_landing_page(slug: str, db: Session = Depends(get_db)) -> LandingPageContent:
    page = db.scalar(
        select(LandingPageContent).where(
            LandingPageContent.slug == slug,
            LandingPageContent.is_published.is_(True),
        )
    )
    if page is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Лендинг не найден"
        )
    return page
